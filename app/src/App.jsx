...
export default function App() {
  ...
  // Mouse Interaction (Commander Target Lock)
  useEffect(() => {
    ...
    const handleCanvasClick = (e) => {
      ...
      if (clickedTargetId) {
          ...
          setAdvisorText(`⚠️ COMMANDER OVERRIDE: TRACKING TARGET ${clickedTargetId}.`);
      } else {
          setPriorityTargetId(null);
  ...
  const handleReset = (newMode = radarMode) => {
    setPriorityTargetId(null);
    setInterceptors([]);
    setContacts([spawnContact(newMode === 'NAVY'), spawnContact(newMode === 'NAVY'), spawnContact(newMode === 'NAVY')]);
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      if (time - lastTime > 30) {
        ...
        setContacts(prevContacts => {
          let updatedContacts = prevContacts.map(contact => {
            // If destroyed, clear after 2 seconds and respawn
            ...
            // Real movement: Keep flying along trajectory
            let newX = contact.x + contact.vx;
            let newY = contact.y + contact.vy;

            // If they fly off-screen completely, despawn and spawn new
          ...
          setInterceptors(prevInterceptors => {
            ...
            const activeContacts = updatedContacts.filter(c => c.status === 'ACTIVE' && Math.hypot(c.x - basePos.x, c.y - basePos.y) <= radarRadius);
            
            if (isAIActive && activeContacts.length > 0) {
              // Target priority or closest
              ...
              // Auto-fire if within engagement zone (200 NM)
              if (targetDist < 200 && Date.now() - lastAutoFire.current > 1500) {
                newInterceptors.push({ x: basePos.x, y: basePos.y, speed: 18, targetId: targetContact.id });
                lastAutoFire.current = Date.now();
                setAdvisorText(`💥 AUTO-DEFENSE ENGAGED TARGET ${targetContact.id}`);
              }
            }
            ...
            return newInterceptors.map(inter => {
              ...
              if (!tEnemy) return null; // Target already dead or gone

              const mdist = Math.hypot(tEnemy.x - inter.x, tEnemy.y - inter.y);
              if (mdist < 15) {
                // HIT! Update the actual contact in the outer scope reference
                updatedContacts = updatedContacts.map(c => c.id === tEnemy.id ? { ...c, status: 'DESTROYED', vx: 0, vy: 0, deathTimer: 0 } : c);
                return null;
              }
      ...
      if (canvas) {
        ...
        // Inner Engagement Zone Ring
        ctx.beginPath(); ctx.arc(cx, cy, 200, 0, Math.PI * 2);
        ctx.strokeStyle = 'rgba(239, 68, 68, 0.3)'; ctx.setLineDash([5, 5]); ctx.stroke(); ctx.setLineDash([]);
        ...
        contacts.forEach(contact => {
          ...
          const color = isActive ? (isMarked ? '#ef4444' : '#facc15') : '#475569';
          ...
          if (isActive) {
            ...
            if (isMarked) {
                ...
                ctx.rotate(-Date.now()/500); // reset rotate for text
            } else {
          ...
          // Contact Velocity Vector Line (Tail)
          if (isActive) {
             ctx.beginPath();
             ctx.moveTo(0,0);
             ctx.lineTo(contact.vx * 15, contact.vy * 15);
          ...
          if (isActive) {
            ...
            drawTacticalText(`SPD: ${Math.floor(contact.speed * 200)} KTS`, contact.x + 45, contact.y - 3, '#94a3b8');
            drawTacticalText(`FL:  ${contact.alt}`, contact.x + 45, contact.y + 7, '#94a3b8');
          } else {
  ...
  return (
    <div style={{ backgroundColor: '#000000', color: 'white', minHeight: '100vh', padding: '20px', fontFamily: '"Courier New", monospace', backgroundImage: 'radial-gradient(circle, #0f172a 0%, #000000 90%)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '250px 1fr 280px', gap: '25px', flex: 1, alignItems: 'stretch' }}>
        ...
        {/* RIGHT PANEL - TRACKING DASHBOARD */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '12px', fontSize: '12px' }}>
              ...
              <div style={{ backgroundColor: '#451a1a', padding: '12px', borderRadius: '6px', marginTop: '8px' }}>
                ...
                {activeThreats.map(c => {
                  ...
                  const col = isMarked ? '#ef4444' : '#facc15';
                  ...
                  return (
                    <div key={c.id} style={{ marginBottom: '10px', paddingBottom: '8px', borderBottom: '1px dashed #7f1d1d' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: col, fontWeight: 'bold' }}>
                        <span>{c.id}</span> <span>{isMarked ? 'LOCKED' : 'TRACKING'}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: '#fca5a5', marginTop: '5px' }}>
                        <span>HDG: {Math.floor(c.hdg)}°</span> <span>SPD: {Math.floor(c.speed * 200)}</span>
                      </div>
                    </div>
                  );
                })}
                ...
                <div style={{ color: '#94a3b8', fontSize: '10px', marginTop: '10px', textAlign: 'center', borderTop: '1px dashed #7f1d1d', paddingTop: '5px' }}>
                    Click target dot to OVERRIDE LOCK.
                </div>
              </div>
            </div>
          </div>
